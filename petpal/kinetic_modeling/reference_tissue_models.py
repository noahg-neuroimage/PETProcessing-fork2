...
def calc_srtm_tac(tac_times: np.ndarray,
    ...
    second_term = (k2 - r1 * bp_coeff) * \
        tcms_conv.calc_convolution_with_check(
            f=exp_term, g=ref_tac_vals, dt=dt)
...
def _calc_simplified_frtm_tac(tac_times: np.ndarray,
    ...
    """
    first_term = r1 * ref_tac_vals
    exp_funcs = a1 * np.exp(-alpha_1 * tac_times) + \
        a2 * np.exp(-alpha_2 * tac_times)
    dt = tac_times[1] - tac_times[0]
    second_term = tcms_conv.calc_convolution_with_check(
        f=exp_funcs, g=ref_tac_vals, dt=dt)
    return first_term + second_term
...
def calc_frtm_tac(tac_times: np.ndarray,
    ...
    """
    r1_n, a1, a2, alpha_1, alpha_2 = _calc_frtm_params_from_kinetic_params(r1=r1,
                                                                           k2=k2,
                                                                           k3=k3,
                                                                           k4=k4)
...
def fit_srtm2_to_tac(tgt_tac_vals: np.ndarray,
    ...
    def _fitting_srtm(tac_times, r1, bp):
        return calc_srtm_tac(tac_times=tac_times,
                             ref_tac_vals=ref_tac_vals,
                             r1=r1,
                             k2=k2_prime,
                             bp=bp)
...
def fit_srtm2_to_tac_with_bounds(tgt_tac_vals: np.ndarray,
    ...
    return sp_fit(f=_fitting_srtm,
                  xdata=ref_tac_times,
                  ydata=tgt_tac_vals,
                  p0=st_values,
                  bounds=[lo_values, hi_values])
...
def fit_mrtm_2003_to_tac(tgt_tac_vals: np.ndarray,
    ...
    t_thresh = get_index_from_threshold(times_in_minutes=ref_tac_times,
                                        t_thresh_in_minutes=t_thresh_in_mins)
    if t_thresh == -1:
        return np.asarray([np.nan, np.nan, np.nan])
    ...
    y = tgt_tac_vals
    x_matrix = np.ones((len(y), 3), float)
    x_matrix[:, 0] = cum_trapz(
        xdata=ref_tac_times, ydata=ref_tac_vals, initial=0.0)
    x_matrix[:, 1] = cum_trapz(
