...
class RTMAnalysis:
    ...
    def run_analysis(self,
        ...
        Returns:
            None
        """
        self.validate_analysis_inputs(
            k2_prime=k2_prime, t_thresh_in_mins=t_thresh_in_mins)
    ...
    def validate_analysis_inputs(self, k2_prime, t_thresh_in_mins):
        ...
        if self.method.startswith("mrtm") and t_thresh_in_mins is None:
            raise ValueError(
                "t_thresh_in_mins must be set for the MRTM analyses.")
    ...
    def calculate_fit(self,
        ...
        ref_tac_times, ref_tac_vals = safe_load_tac(
            filename=self.ref_tac_path, **tac_load_kwargs)
        _tgt_tac_times, tgt_tac_vals = safe_load_tac(
            filename=self.roi_tac_path, **tac_load_kwargs)
    ...
    def calculate_fit_properties(self,
        ...
        if self.method.startswith("frtm") or self.method.startswith("srtm"):
            self._calc_frtm_or_srtm_fit_props(
                fit_results=fit_results, k2_prime=k2_prime)
        else:
    ...
    def _calc_mrtm_fit_props(self, fit_results: np.ndarray,
        ...
        t_thresh_index = get_index_from_threshold(
            times_in_minutes=ref_tac_times, t_thresh_in_minutes=t_thresh_in_mins)
    ...
    def _calc_frtm_or_srtm_fit_props(self,
        ...
        if self.method.endswith('2'):
            ...
            self.analysis_props["FitValues"] = format_func(
                fit_params.round(5), True)
            self.analysis_props["FitStdErr"] = format_func(
                fit_stderr.round(5), True)
        else:
            self.analysis_props["FitValues"] = format_func(
                fit_params.round(5), False)
            self.analysis_props["FitStdErr"] = format_func(
                fit_stderr.round(5), False)
    ...
    def _get_pretty_frtm_fit_param_vals(param_fits: np.ndarray, reduced: bool = False) -> dict:
        ...
        Returns a dictionary with keys: 'R1', 'k2', 'k3', and 'k4' and the corresponding values
        from ``param_fits``.
