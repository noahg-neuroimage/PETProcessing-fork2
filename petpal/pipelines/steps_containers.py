"""
Manages containers of steps for pipelines.
"""
import copy
from typing import Union
import networkx as nx
from matplotlib import pyplot as plt
from .steps_base import FunctionBasedStep, ObjectBasedStep
...
class StepsContainer:
    ...
    def remove_step(self, step: Union[int, str]):
        ...
        if isinstance(step, int):
            try:
                ...
                raise IndexError(f"Step number {step} does not exist.") from exc
    ...
    def __call__(self):
        ...
        """
        for _step_id, (_step_name, a_step) in enumerate(zip(self.step_names, self.step_objs)):
            a_step.execute()
    ...
    def __getitem__(self, step: Union[int, str]):
        ...
        if isinstance(step, int):
            ...
            try:
                ...
                raise KeyError(f"Step name {step} does not exist.") from exc
        else:
...
class StepsPipeline:
    ...
    def __str__(self) -> str:
        ...
        for con_name, con_obj in self.step_containers.items():
            ...
            for step_num, step_name in enumerate(con_obj.step_names):
                ...
                if step_deps:
                    info_str.append(r"      \> sends output to:")
    ...
    def add_container(self, step_container: StepsContainer):
        ...
        if container_name in self.step_containers:
    ...
    def add_step(self, container_name: str, step: StepType):
        ...
        """
        if container_name not in self.step_containers:
    ...
    def print_steps_names(self, container_name: Union[str, None] = None):
        ...
        if container_name is None:
            for _name, container in self.step_containers.items():
                print(str(container))
        elif container_name in self.step_containers:
    ...
    def can_steps_potentially_run(self) -> bool:
        ...
        for _step_name, run_state in steps_run_state.items():
            if not run_state:
                return False
        return True
    ...
    def plot_dependency_graph(self,
        ...
        _myFig, myAx = plt.subplots(figsize=figsize, constrained_layout=True)
